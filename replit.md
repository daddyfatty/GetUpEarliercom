...
```
Changelog:
- July 22, 2025: Implemented Klaviyo email collection integration with fallback system - replaced custom React newsletter signup forms in both footer and purple gradient newsletter popup with Klaviyo form class "klaviyo-form-ULBmqZ"; added Klaviyo JavaScript library to HTML head; created fallback email collection system that attempts Klaviyo API integration and falls back to mailto link for manual processing; maintained same visual structure and messaging while providing functional email signup across all pages
