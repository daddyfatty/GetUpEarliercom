...
export default function AmazonProductsPage() {
  ...
  const [otherProducts, setOtherProducts] = useState<AmazonProduct[]>([]);
  ...
  useEffect(() => {
    const fetchAmazonProducts = async () => {
      try {
        ...
        const other: AmazonProduct[] = [];
        
        // Keywords to identify single ingredient supplements
        const singleIngredientKeywords = [
          'pea protein', 'whey protein', 'collagen peptides', 'creatine monohydrate',
          'protein isolate', 'grass fed whey', 'single ingredient', 'naked whey'
        ];
        
        validProducts.forEach(product => {
          const titleLower = product.title.toLowerCase();
          const isSingleIngredient = singleIngredientKeywords.some(keyword => 
            titleLower.includes(keyword)
          );
          
          if (isSingleIngredient) {
            singleIngredient.push(product);
          } else {
            other.push(product);
          }
        });
        ...
        setSingleIngredientProducts(singleIngredient);
        ...
        <div className="max-w-4xl mx-auto">
          ...
          {products.length === 0 ? (
            ...
            <div className="space-y-12">
              {/* Single Ingredient Supplements Section */}
              {singleIngredientProducts.length > 0 && (
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 mb-6">Single Ingredient Supplements</h2>
                  <div className="space-y-8">
                    {singleIngredientProducts.map((product, index) => (
                      <ProductCard key={`single-${index}`} product={product} />
                    ))}
                  </div>
                </div>
              )}
              ...
              {otherProducts.length > 0 && (
                <div>
                  {singleIngredientProducts.length > 0 && (
                    <h2 className="text-2xl font-bold text-gray-900 mb-6">Other Recommendations</h2>
                  )}
                  <div className="space-y-8">
                    {otherProducts.map((product, index) => (
                      <ProductCard key={`other-${index}`} product={product} />
                    ))}
                  </div>
                </div>
              )}
