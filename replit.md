...
```
Changelog:
- July 20, 2025: Perfected full-width image gallery system with 2-column layout - images now completely fill the post space edge-to-edge using container breakouts; single images utilize full available width, multiple images display in clean 2-column grid for optimal space usage; enhanced lightbox with 95vw/95vh sizing; wide landscape images (like Strava screenshots) now properly stretch across entire available area without padding constraints
