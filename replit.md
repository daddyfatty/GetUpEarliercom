...
```
Changelog:
- July 20, 2025: Perfected full-width image gallery system with single-column layout - images now completely fill the post space edge-to-edge using container breakouts; both single and multiple images utilize full available width with single-column vertical stacking for optimal space usage; enhanced lightbox with 95vw/95vh sizing; wide landscape images (like Strava screenshots) now properly stretch across entire available area without padding constraints
