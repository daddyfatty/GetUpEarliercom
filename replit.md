...
```
Changelog:
- July 20, 2025: Perfected full-width image gallery system - images now completely fill the bottom of posts using proper negative margins and container breakouts; single images span full card width, multiple images use true masonry layout with responsive columns (1/2/3/4 based on screen size); enhanced lightbox with 95vw/95vh sizing and semi-transparent close button; wide landscape images (like Strava screenshots) now properly utilize full available width for optimal viewing experience
